...
func Recovery(shares []string) (string, error) {
	// Recreate the original object of x, y points, based upon number of shares
	// and size of each share (number of parts in the secret).
	var secrets [][][]*big.Int = make([][][]*big.Int, len(shares))
	...
	for {
		for j := range secret {
			...
			secret[j] = big.NewInt(0)
			// secret[j], _ = big.NewInt(0).SetString(number.String(), 10)
			// ...and every share...
			for i := range secrets { // LPI sum loop
				// L_i를 이야기함
				origin := secrets[i][j][0]
				...
				for k := range secrets { // LPI product loop
					if k != i {
						...
						// x-x_k를 이야기함
						// negative, _ := big.NewInt(0).SetString(number.String(), 10)
						negative := big.NewInt(0)
						negative = negative.Mul(current, big.NewInt(-1))
						negative = negative.Add(negative, number)
						negative = negative.Mod(negative, prime)
